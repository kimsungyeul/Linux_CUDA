...
int main(int argc, char **argv)
{
	...
	uint8x16_t fill3 = vmovq_n_u8(3);	/* 모두 5로채워진 16비트 벡터를 생성 */

	data = vld1q_u8(source);			/* 데이터셋을 벡터에 대입 */

	printf("data : ");
	printData(data);					/* 벡터에 저장된 값을 출력 */

	ret = vaddq_u8(data, fill3);		/* 두 벡터의 요소들을 각각 더함 */
	printf("Add: ");
	printData(ret);						/* 벡터에 저장된 값을 출력*/ 

	ret = vsubq_u8(data, fill3);		/* 두 벡터의 요소들을 각각 더함 */
	printf("Subtract: ");
	printData(ret);						/* 벡터에 저장된 값을 출력*/ 

	ret = vmulq_u8(data, fill3);		/* 두 벡터의 요소들을 각각 더함 */
	printf("Multiply: ");
	printData(ret);						/* 벡터에 저장된 값을 출력*/ 
