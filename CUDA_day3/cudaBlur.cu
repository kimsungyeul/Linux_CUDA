...
__global__ void convertToBlur(ubyte *rgb, ubyte *out, int rows, int cols, int elemSize) {
	...
	if (col < cols && row < rows) {
		//int blur_offset = row * cols + col;
		//int rgb_offset = blur_offset * elemSize;

		for(int z = 0; z < elemSize; z++){
		for(int i = -1; i < 2; i++) {
			for(int j = -1; j < 2; j++){
				sum += kernel[i+1][j+1]*rgb[(col-i*elemSize) + (row-j)*cols*elemSize + z];
			}
		}
		out[col+row*cols*elemSize] = sum;
		}
